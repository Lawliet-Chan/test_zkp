...
RUN cd /scroll/roller && make libzkp && go test -tags="gpu ffi" -timeout 0 -c prover/prover_test.go
RUN cd /scroll/coordinator && make libzkp && go test -tags="gpu ffi" -timeout 0 -c verifier/verifier_test.go
